...
export default function Export() {
  ...
  return (
    <div className="max-w-2xl space-y-4">
      ...
      <div className="grid grid-cols-[140px_1fr] gap-3 items-center text-sm">
        ...
        <span className="flex gap-4"><label className="flex items-center gap-1"><input type="checkbox" checked={xmp} onChange={(e) => setXmp(e.target.checked)} /> XMP sidecars</label><label className="flex items-center gap-1"><input type="checkbox" checked={tree} onChange={(e) => setTree(e.target.checked)} /> sorted tree</label></span>
      </div>
      <button onClick={() => run.mutate()} disabled={run.isPending} className="px-4 py-1.5 rounded-md bg-blue-600 hover:bg-blue-500 disabled:opacity-40 text-sm font-medium">{run.isPending ? 'Exporting…' : 'Run export'}</button>
      ...
      {result && (
        <div className="rounded-lg border border-gray-800 bg-gray-900 p-3 text-sm">
          <div>Wrote <b>{result.images}</b> records to <code>{result.out}</code>{result.xmp_written ? `, ${result.xmp_written} XMP files` : ''}.</div>
      ...
      {exports && exports.length > 0 && (
        <div>
          ...
          <ul className="text-sm text-gray-400 space-y-0.5">{exports.map((e) => <li key={e.name}><code>{e.path}</code> · {new Date(e.mtime * 1000).toLocaleString()}</li>)}</ul>
        </div>
      )}
    </div>
  );
